...
int fts_create_apk_debug_channel_2(struct fts_ts_data *ts_data)
{
	struct ftxxxx_proc *proc = &ts_data->proc;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0))
...
void fts_release_apk_debug_channel_2(struct fts_ts_data *ts_data)
{
	struct ftxxxx_proc *proc = &ts_data->proc;
...
{
	ssize_t count = 0;
	struct irq_desc *desc = irq_to_desc(fts_data_2->irq);
				  ...
				  const char *buf, size_t count)
{
	...
	if (FTS_SYSFS_ECHO_ON(buf)) {
		...
		fts_data_2->fw_is_running = false;
	} else if (FTS_SYSFS_ECHO_OFF(buf)) {
		FTS_INFO("[EX-FUN]set to fw mode");
		fts_data_2->fw_is_running = true;
	}
	mutex_unlock(&input_dev->mutex);
	FTS_FUNC_EXIT();
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	FTS_FUNC_ENTER();
	mutex_lock(&input_dev->mutex);
	if (true == fts_data_2->fw_is_running)
...
/* fts_upgrade_bin_2 interface */
static ssize_t fts_fwupgrade_2bin_show(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	return -EPERM;
}

static ssize_t fts_fwupgrade_2bin_store(struct device *dev,
				      struct device_attribute *attr,
				      const char *buf, size_t count)
{
			      ...
			      char *buf)
{
	...
	for (i = 0; i < fts_data_2->pnt_buf_size; i++) {
		count +=
		    snprintf(buf + count, PAGE_SIZE, "%02x ",
			     fts_data_2->point_buf[i]);
	}
	count += snprintf(buf + count, PAGE_SIZE, "\n");
	mutex_unlock(&input_dev->mutex);
				   ...
				   const char *buf, size_t count)
{
	...
	mutex_lock(&input_dev->mutex);
	sscanf(buf, "%d", &value);
	FTS_DEBUG("log level:%d->%d", fts_data_2->log_level, value);
	fts_data_2->log_level = value;
	mutex_unlock(&input_dev->mutex);
	FTS_FUNC_EXIT();
...
static DEVICE_ATTR(fts_rw_reg, S_IRUGO | S_IWUSR, fts_tprwreg_show,
		   fts_tprwreg_store);
/*  upgrade from fw bin file   example:echo "*.bin" > fts_upgrade_bin_2 */
static DEVICE_ATTR(fts_upgrade_bin_2, S_IRUGO | S_IWUSR, fts_fwupgrade_2bin_show,
		   fts_fwupgrade_2bin_store);
...
static struct attribute *fts_attributes[] = {
	...
	&dev_attr_fts_upgrade_bin_2.attr,
