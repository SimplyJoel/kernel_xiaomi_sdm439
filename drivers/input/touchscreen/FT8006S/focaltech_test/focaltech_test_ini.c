						 ...
						 char *fwname)
{
	...
	struct device *dev = &fts_data_2->input_dev->dev;
...
static u32 ini_get_ic_code(char *ic_name)
{
	...
	int ic_types_2_len = 0;

	ini_icname_len = strlen(ic_name);
	type_size = sizeof(ic_types_2) / sizeof(ic_types_2[0]);
	for (i = 0; i < type_size; i++) {
		ic_types_2_len = strlen(ic_name);
		if (ini_icname_len == ic_types_2_len) {
			if (0 ==
			    strncmp(ic_name, ic_types_2[i].ic_name, ic_types_2_len))
				return ic_types_2[i].ic_type;
		}
	}
...
/*
 * fts_test_get_testparam_from_ini_2 - get test parameters from ini
 *
 * read, parse the configuration file, initialize the test variable
 *
 * return 0 if succuss, else errro code
 */
int fts_test_get_testparam_from_ini_2(char *config_name)
{
	int ret = 0;
	struct ini_data *ini = &fts_ftest->ini;
